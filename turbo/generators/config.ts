...
const snakeCase = (t: string) => t.replace(/[-_]/g, '_')
const validateDashCase = (i: string) => !/[A-Z]/.test(i) && !/_/.test(i)

const ui = readdirSync('./packages/ui').filter((n) => !n.includes('.'))

const updateComponentsPackage: PlopTypes.CustomActionFunction = async (
  answers,
) => {
  if (!answers) return ''
...
const updateComposablesPackage: PlopTypes.CustomActionFunction = async (
  answers,
) => {
  if (!answers) return ''
...
const generator = (plop: PlopTypes.NodePlopAPI): void => {
  ...
  plop.setGenerator('icon-story', {
    description: 'Create a new icon story',
    prompts: [
      {
        type: 'input',
        name: 'iconName',
        validate: (input: string) => {
          ...
          if (!validateDashCase(input))
            return `Please enter the icon name in kebab case.`
    ...
    actions: [
      {
        type: 'addMany',
        destination: './apps/docs/stories/components/icon',
        templateFiles: 'templates/icon-story/**',
        base: 'templates/icon-story',
        abortOnFail: true,
      },
    ],
  })
